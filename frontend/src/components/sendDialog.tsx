...
export default function SendDialog({ setOpenDialogName, initialHandle } : { setOpenDialogName: any, initialHandle: string }) {
    ...
    const handleInitiateTransaction = async () => {
        if (senderUsername && receiver && sendAmount) {
            try {
                const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/initiateTransaction`, {
                    senderUsername,     
    ...
    const handleConfirm = async () => {
        if (transactionId && senderUsername) {
            try {
                const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/commitTransaction`, {
                    transactionId,
                    senderUsername,
                });
